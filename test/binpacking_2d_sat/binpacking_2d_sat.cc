...
ABSL_FLAG(int, max_bins, 0, "Maximum number of bins. The 0 default value implies the code will "
                            "use some heuristics to compute this number.");
ABSL_FLAG(int, symmetry_breaking_level, 2, "Use symmetry breaking constraints");
ABSL_FLAG(bool, use_global_cumulative, true, "Use a global cumulative relaxation");
...
)
{
    ...
    if (!found_incompatible_pair && !incompatible_pair_candidates.empty())
    {
        ...
        fixed_items.insert(*std::min_element(incompatible_pair_candidates.begin(), incompatible_pair_candidates.end(), GreaterByArea(problem)));
    }
...
void LoadAndSolve(const std::string &file_name, int instance)
{
    ...
    for (int item = 0; item < num_items; ++item)
    {
        for (int dim = 0; dim < num_dimensions; ++dim)
        {
            min_sizes_per_dimension[dim] =
                std::min(min_sizes_per_dimension[dim], problem.items(item).shapes(0).dimensions(dim));
        }
    }
    ...
    if (absl::GetFlag(FLAGS_symmetry_breaking_level) >= 2)
    {
        ...
        for (const auto &[dim, item_indexes] : item_indexes_for_dimensions)
        {
            ...
            for (int i = 1; i < item_indexes.size(); ++i)
            {
                ...
                cp_model.AddLessOrEqual(prev_start_x * bin_sizes[1] + prev_start_y, curr_start_x * bin_sizes[1] + curr_start_y);
            }
        }
    ...
    if (absl::GetFlag(FLAGS_symmetry_breaking_level) >= 1)
    {
        ...
        std::sort(not_placed_items.begin(), not_placed_items.end(), GreaterByArea(problem));
...
int main(int argc, char **argv)
{
    ...
    operations_research::sat::LoadAndSolve(absl::GetFlag(FLAGS_input), absl::GetFlag(FLAGS_instance));
    return EXIT_SUCCESS;
}
