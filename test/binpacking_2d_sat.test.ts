...
test('ts-ortools', () =>
{
  ...
  //             {
  //                 if ( 2 * problem.items( i ).shapes( 0 ).dimensions( 0 ) > bin_sizes[ 0 ] && 2 * problem.items( i ).shapes( 0 ).dimensions( 1 ) > bin_sizes[ 1 ] )
  //                 {
  ...
  //         if ( bin_sizes[ 0 ] == original_bin_sizes[ 0 ] && bin_sizes[ 1 ] == original_bin_sizes[ 1 ] )
  //         {
  ...
  let params = "";
  let max_bins = 0;
  let symmetry_breaking_level = 2;
  let use_global_cumulative = true;

  class GreaterByArea
  {
    constructor(private problem_: operations_research.packing.MultipleDimensionsBinPackingProblem)
    {
    }

     operator( a: number,  b: number): boolean
     {
        const a_sizes = this.problem_.items( a ).shapes( 0 ).dimensions();
        const b_sizes = this.problem_.items( b ).shapes( 0 ).dimensions();
        return a_sizes[ 0 ] * a_sizes[ 1 ] > b_sizes[ 0 ] * b_sizes[ 1 ];
     }
  }

  function ItemsAreIncompatible(problem: operations_research.packing.MultipleDimensionsBinPackingProblem, i1: number , i2: number): boolean
  {
    ...
    return (i1_sizes[ 0 ] + i2_sizes[ 0 ] > bin_sizes[ 0 ]) && (i1_sizes[ 1 ] + i2_sizes[ 1 ] > bin_sizes[ 1 ]);
  }
