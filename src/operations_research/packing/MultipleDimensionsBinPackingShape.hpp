...
class GMultipleDimensionsBinPackingShape : public Napi::ObjectWrap<GMultipleDimensionsBinPackingShape>
{
  public:
    ...
    Napi::Value dimensions_size(const Napi::CallbackInfo &info)
    {
        ...
        if (info.Length() == 0)
        {
            return Napi::Number::New(env, pMultipleDimensionsBinPackingShape->dimensions_size());
        }

        Napi::TypeError::New(env, "operations_research::packing::GMultipleDimensionsBinPackingShape::dimensions_size : Invalid arguments").ThrowAsJavaScriptException();
        return env.Null();
    }
