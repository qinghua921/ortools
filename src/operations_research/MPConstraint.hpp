...
class GMPConstraint : public Napi::ObjectWrap<GMPConstraint>
{
  public:
    ...
    Napi::Value SetCoefficient(const Napi::CallbackInfo &info)
    {
        ...
        if (info.Length() == 2 &&
            info[0].IsObject() && info[0].As<Napi::Object>().InstanceOf(GMPVariable::constructor.Value()) &&
            info[1].IsNumber())
        {
            GMPVariable *pGMPVariable = Napi::ObjectWrap<GMPVariable>::Unwrap(info[0].As<Napi::Object>());
