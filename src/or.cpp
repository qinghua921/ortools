...
Napi::Value GMPSolver::Solve(const Napi::CallbackInfo &info)
{
    ...
    {
        GMPSolverParameters *param    = GMPSolverParameters::Unwrap(info[0].As<Napi::Object>());
...
Napi::Value GMPSolver::FillSolutionResponseProto(const Napi::CallbackInfo &info)
{
    ...
    {
        GMPSolutionResponse *response = GMPSolutionResponse::Unwrap(info[0].As<Napi::Object>());
...
Napi::Value GMPSolver::SetHint(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 && info[0].IsArray())
    {
        ...
        for (int i = 0; i < hint_array.Length(); i++)
        {
            ...
            GMPVariable *variable = GMPVariable::Unwrap(variable_object);
...
Napi::Value GMPSolver::GetMPModelRequestLoggingInfo(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 && info[0].IsObject() && info[0].As<Napi::Object>().InstanceOf(GMPModelRequest::constructor.Value()))
    {
        ...
        GMPModelRequest *request    = GMPModelRequest::Unwrap(request_object);
...
Napi::Value GMPCallback::RunCallback(const Napi::CallbackInfo &info)
{
    ...
    {
        GMPCallbackContext *callback_context = GMPCallbackContext::Unwrap(info[0].As<Napi::Object>());
    ...
    : Napi::ObjectWrap<GLinearRange>(info)
{
    ...
    if (info.Length() == 3 && info[0].IsNumber() && GLinearExpr::ToLinearExpr(info[1], linear_expr) && info[2].IsNumber())
    {
        double lower_bound = info[0].As<Napi::Number>().DoubleValue();
        double upper_bound = info[2].As<Napi::Number>().DoubleValue();
        pLinearRange       = new LinearRange(lower_bound, linear_expr, upper_bound);
        return;
    }
...
Napi::Value GMPConstraint::SetCoefficient(const Napi::CallbackInfo &info)
{
    ...
    {
        GMPVariable *var = GMPVariable::Unwrap(info[0].As<Napi::Object>());
...
Napi::Value GMPObjective::OptimizeLinearExpr(const Napi::CallbackInfo &info)
{
    ...
    LinearExpr linear_expr;
    if (info.Length() == 2 && GLinearExpr::ToLinearExpr(info[0], linear_expr) && info[1].IsBoolean())
    {
        bool is_maximization = info[1].As<Napi::Boolean>().Value();
        this->pMPObjective->OptimizeLinearExpr(linear_expr, is_maximization);
        return env.Undefined();
    }
...
Napi::Value GMPObjective::MaximizeLinearExpr(const Napi::CallbackInfo &info)
{
    ...
    LinearExpr linear_expr;
    if (info.Length() == 1 && GLinearExpr::ToLinearExpr(info[0], linear_expr))
    {
        this->pMPObjective->MaximizeLinearExpr(linear_expr);
        return env.Undefined();
    }
...
Napi::Value GMPObjective::MinimizeLinearExpr(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 && GLinearExpr::ToLinearExpr(info[0], linear_expr))
    {
        this->pMPObjective->MinimizeLinearExpr(linear_expr);
        return env.Undefined();
    }
...
Napi::Value GMPObjective::AddLinearExpr(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 && GLinearExpr::ToLinearExpr(info[0], linear_expr))
    {
        this->pMPObjective->AddLinearExpr(linear_expr);
        return env.Undefined();
    }
...
Napi::Value GLinearExpr::NotVar(const Napi::CallbackInfo &info)
{
    ...
    LinearExpr var;
    if (info.Length() == 1 && ToLinearExpr(info[0], var))
    {
        LinearExpr result = LinearExpr::NotVar(var);
        return GLinearExpr::constructor.New({Napi::External<LinearExpr>::New(env, new LinearExpr(result))});
    }
    ...
    : Napi::ObjectWrap<GDomain>(info)
{
    ...
    Napi::TypeError::New(env, "GDomain::GDomain : Invalid arguments").ThrowAsJavaScriptException();
}
...
Napi::Object GDomain::Init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(
        ...
        "Domain",
        {
            ...
            StaticMethod("FromIntervals", &GDomain::FromIntervals),
            StaticMethod("FromFlatSpanOfIntervals", &GDomain::FromFlatSpanOfIntervals),
            StaticMethod("FromVectorIntervals", &GDomain::FromVectorIntervals),
            StaticMethod("FromFlatIntervals", &GDomain::FromFlatIntervals),
            InstanceMethod("FlattenedIntervals", &GDomain::FlattenedIntervals),
            InstanceMethod("IsEmpty", &GDomain::IsEmpty),
            InstanceMethod("Size", &GDomain::Size),
            InstanceMethod("HasTwoValues", &GDomain::HasTwoValues),
            InstanceMethod("Min", &GDomain::Min),
            InstanceMethod("Max", &GDomain::Max),
            InstanceMethod("SmallestValue", &GDomain::SmallestValue),
            InstanceMethod("ClosestValue", &GDomain::ClosestValue),
            InstanceMethod("ValueAtOrBefore", &GDomain::ValueAtOrBefore),
            InstanceMethod("ValueAtOrAfter", &GDomain::ValueAtOrAfter),
            InstanceMethod("PartAroundZero", &GDomain::PartAroundZero),
            InstanceMethod("IsFixed", &GDomain::IsFixed),
            InstanceMethod("FixedValue", &GDomain::FixedValue),
            InstanceMethod("Contains", &GDomain::Contains),
            InstanceMethod("Distance", &GDomain::Distance),
            InstanceMethod("IsIncludedIn", &GDomain::IsIncludedIn),
            InstanceMethod("Complement", &GDomain::Complement),
            InstanceMethod("Negation", &GDomain::Negation),
            InstanceMethod("IntersectionWith", &GDomain::IntersectionWith),
            InstanceMethod("UnionWith", &GDomain::UnionWith),
            InstanceMethod("AdditionWith", &GDomain::AdditionWith),
            InstanceMethod("MultiplicationBy", &GDomain::MultiplicationBy),
            InstanceMethod("RelaxIfTooComplex", &GDomain::RelaxIfTooComplex),
            InstanceMethod("ContinuousMultiplicationBy", &GDomain::ContinuousMultiplicationBy),
            InstanceMethod("DivisionBy", &GDomain::DivisionBy),
            InstanceMethod("InverseMultiplicationBy", &GDomain::InverseMultiplicationBy),
            InstanceMethod("PositiveModuloBySuperset", &GDomain::PositiveModuloBySuperset),
            InstanceMethod("PositiveDivisionBySuperset", &GDomain::PositiveDivisionBySuperset),
            InstanceMethod("SquareSuperset", &GDomain::SquareSuperset),
            InstanceMethod("SimplifyUsingImpliedDomain", &GDomain::SimplifyUsingImpliedDomain),
            InstanceMethod("ToString", &GDomain::ToString),
            InstanceMethod("operator_lt", &GDomain::operator_lt),
            InstanceMethod("operator_eq", &GDomain::operator_eq),
            InstanceMethod("operator_nq", &GDomain::operator_nq),
            InstanceMethod("NumIntervals", &GDomain::NumIntervals),
            InstanceMethod("operator_brackets", &GDomain::operator_brackets),
        }
    );
    constructor = Napi::Persistent(func);
    constructor.SuppressDestruct();
...
Napi::Value GDomain::FromFlatSpanOfIntervals(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsArray())
    {
        ...
        for (uint32_t i = 0; i < array.Length(); i++)
        {
            ...
            if (!value.IsNumber())
            {
                Napi::TypeError::New(env, "GDomain::FromFlatSpanOfIntervals : Invalid arguments").ThrowAsJavaScriptException();
                return env.Null();
            }
            vec.push_back(value.As<Napi::Number>().Int64Value());
        }
        Domain result = Domain::FromFlatSpanOfIntervals(vec);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
...
Napi::Value GDomain::FromVectorIntervals(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsArray())
    {
        Napi::Array intervals = info[0].As<Napi::Array>();
        std::vector<std::vector<int64_t>> vec;
        for (uint32_t i = 0; i < intervals.Length(); i++)
        {
            auto interval = intervals.Get(i);
            if (!interval.IsArray())
            {
                Napi::TypeError::New(env, "GDomain::FromVectorIntervals : Invalid arguments").ThrowAsJavaScriptException();
                return env.Null();
            }
            std::vector<int64_t> subvec;
            for (uint32_t j = 0; j < interval.As<Napi::Array>().Length(); j++)
            {
                auto value = interval.As<Napi::Array>().Get(j);
                if (!value.IsNumber())
                {
                    Napi::TypeError::New(env, "GDomain::FromVectorIntervals : Invalid arguments").ThrowAsJavaScriptException();
                    return env.Null();
                }
                subvec.push_back(value.As<Napi::Number>().Int64Value());
            }
            vec.push_back(subvec);
        }
        Domain result = Domain::FromVectorIntervals(vec);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }
...
Napi::Value GDomain::FromFlatIntervals(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsArray())
    {
        ...
        for (uint32_t i = 0; i < intervals.Length(); i++)
        {
            auto value = intervals.Get(i);
            if (!value.IsNumber())
            {
                Napi::TypeError::New(env, "GDomain::FromFlatIntervals : Invalid arguments").ThrowAsJavaScriptException();
                return env.Null();
            }
            vec.push_back(value.As<Napi::Number>().Int64Value());
        }
        Domain result = Domain::FromFlatIntervals(vec);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }
...
Napi::Value GDomain::FlattenedIntervals(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 0)
    {
        std::vector<int64_t> result = pDomain->FlattenedIntervals();
        Napi::Array array           = Napi::Array::New(env, result.size());
        for (uint32_t i = 0; i < result.size(); i++)
        {
            array.Set(i, Napi::Number::New(env, result[i]));
        }
        return array;
    }

    Napi::TypeError::New(env, "GDomain::FlattenedIntervals : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}
// bool IsEmpty() const;
Napi::Value GDomain::IsEmpty(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Boolean::New(env, pDomain->IsEmpty());
    }

    Napi::TypeError::New(env, "GDomain::IsEmpty : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}
// int64_t Size() const;
Napi::Value GDomain::Size(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->Size());
    }

    Napi::TypeError::New(env, "GDomain::Size : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// bool HasTwoValues() const;
Napi::Value GDomain::HasTwoValues(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Boolean::New(env, pDomain->HasTwoValues());
    }

    Napi::TypeError::New(env, "GDomain::HasTwoValues : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t Min() const;
Napi::Value GDomain::Min(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->Min());
    }

    Napi::TypeError::New(env, "GDomain::Min : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t Max() const;
Napi::Value GDomain::Max(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->Max());
    }

    Napi::TypeError::New(env, "GDomain::Max : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t SmallestValue() const;
Napi::Value GDomain::SmallestValue(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->SmallestValue());
    }

    Napi::TypeError::New(env, "GDomain::SmallestValue : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t ClosestValue(int64_t wanted) const;
Napi::Value GDomain::ClosestValue(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsNumber())
    {
        int64_t wanted = info[0].As<Napi::Number>().Int64Value();
        int64_t result = pDomain->ClosestValue(wanted);
        return Napi::Number::New(env, result);
    }

    Napi::TypeError::New(env, "GDomain::ClosestValue : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t ValueAtOrBefore(int64_t input) const;
Napi::Value GDomain::ValueAtOrBefore(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsNumber())
    {
        ...
        int64_t result = pDomain->ValueAtOrBefore(input);
        return Napi::Number::New(env, result);
    }

    Napi::TypeError::New(env, "GDomain::ValueAtOrBefore : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t ValueAtOrAfter(int64_t input) const;
Napi::Value GDomain::ValueAtOrAfter(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        int64_t input  = info[0].As<Napi::Number>().Int64Value();
        int64_t result = pDomain->ValueAtOrAfter(input);
        return Napi::Number::New(env, result);
    }

    Napi::TypeError::New(env, "GDomain::ValueAtOrAfter : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain PartAroundZero() const;
Napi::Value GDomain::PartAroundZero(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = pDomain->PartAroundZero();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::PartAroundZero : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// bool IsFixed() const;
Napi::Value GDomain::IsFixed(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Boolean::New(env, pDomain->IsFixed());
    }

    Napi::TypeError::New(env, "GDomain::IsFixed : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t FixedValue() const;
Napi::Value GDomain::FixedValue(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->FixedValue());
    }

    Napi::TypeError::New(env, "GDomain::FixedValue : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}
...
Napi::Value GDomain::Contains(const Napi::CallbackInfo &info)
{
    ...
    {
        int64_t value = info[0].As<Napi::Number>().Int64Value();
        return Napi::Boolean::New(env, pDomain->Contains(value));
    }

    Napi::TypeError::New(env, "GDomain::Contains : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// int64_t Distance(int64_t value) const;
Napi::Value GDomain::Distance(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        int64_t value  = info[0].As<Napi::Number>().Int64Value();
        int64_t result = pDomain->Distance(value);
        return Napi::Number::New(env, result);
    }

    Napi::TypeError::New(env, "GDomain::Distance : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// bool IsIncludedIn(const Domain &domain) const;
Napi::Value GDomain::IsIncludedIn(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other = GDomain::Unwrap(info[0].As<Napi::Object>());
        return Napi::Boolean::New(env, pDomain->IsIncludedIn(*other->pDomain));
    }

    Napi::TypeError::New(env, "GDomain::IsIncludedIn : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain Complement() const;
Napi::Value GDomain::Complement(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = pDomain->Complement();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::Complement : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain Negation() const;
Napi::Value GDomain::Negation(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = pDomain->Negation();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::Negation : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain IntersectionWith(const Domain &domain) const;
Napi::Value GDomain::IntersectionWith(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->IntersectionWith(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::IntersectionWith : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain UnionWith(const Domain &domain) const;
Napi::Value GDomain::UnionWith(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->UnionWith(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::UnionWith : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain AdditionWith(const Domain &domain) const;
Napi::Value GDomain::AdditionWith(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->AdditionWith(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::AdditionWith : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain MultiplicationBy(int64_t coeff, bool *exact = nullptr) const;
Napi::Value GDomain::MultiplicationBy(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsNumber())
    {
        int64_t coeff = info[0].As<Napi::Number>().Int64Value();
        bool exact    = false;
        Domain result = pDomain->MultiplicationBy(coeff, &exact);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        auto ret      = Napi::Object::New(env);
        ret.Set("return", GDomain::constructor.New({external}));
        ret.Set("exact", Napi::Boolean::New(env, exact));
        return ret;
    }

    Napi::TypeError::New(env, "GDomain::MultiplicationBy : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain RelaxIfTooComplex() const;
Napi::Value GDomain::RelaxIfTooComplex(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = pDomain->RelaxIfTooComplex();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::RelaxIfTooComplex : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain ContinuousMultiplicationBy(int64_t coeff) const;
// Domain ContinuousMultiplicationBy(const Domain &domain) const;
Napi::Value GDomain::ContinuousMultiplicationBy(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        int64_t coeff = info[0].As<Napi::Number>().Int64Value();
        Domain result = pDomain->ContinuousMultiplicationBy(coeff);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->ContinuousMultiplicationBy(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::ContinuousMultiplicationBy : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain DivisionBy(int64_t coeff) const;
Napi::Value GDomain::DivisionBy(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        int64_t coeff = info[0].As<Napi::Number>().Int64Value();
        Domain result = pDomain->DivisionBy(coeff);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::DivisionBy : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain InverseMultiplicationBy(int64_t coeff) const;
Napi::Value GDomain::InverseMultiplicationBy(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        int64_t coeff = info[0].As<Napi::Number>().Int64Value();
        Domain result = pDomain->InverseMultiplicationBy(coeff);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::InverseMultiplicationBy : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain PositiveModuloBySuperset(const Domain &modulo) const;
Napi::Value GDomain::PositiveModuloBySuperset(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->PositiveModuloBySuperset(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::PositiveModuloBySuperset : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain PositiveDivisionBySuperset(const Domain &divisor) const;
Napi::Value GDomain::PositiveDivisionBySuperset(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other    = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result = pDomain->PositiveDivisionBySuperset(*other->pDomain);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::PositiveDivisionBySuperset : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain SquareSuperset() const;
Napi::Value GDomain::SquareSuperset(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = pDomain->SquareSuperset();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::SquareSuperset : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// Domain SimplifyUsingImpliedDomain(const Domain &implied_domain) const;
Napi::Value GDomain::SimplifyUsingImpliedDomain(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsObject() && info[0].As<Napi::Object>().InstanceOf(GDomain::constructor.Value()))
    {
        auto implied_domain = GDomain::Unwrap(info[0].As<Napi::Object>());
        Domain result       = pDomain->SimplifyUsingImpliedDomain(*implied_domain->pDomain);
        auto external       = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::SimplifyUsingImpliedDomain : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// std::string ToString() const;
Napi::Value GDomain::ToString(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        std::string result = pDomain->ToString();
        return Napi::String::New(env, result);
    }

    Napi::TypeError::New(env, "GDomain::ToString : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// bool operator<(const Domain &other) const;
Napi::Value GDomain::operator_lt(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        auto other = GDomain::Unwrap(info[0].As<Napi::Object>());
        return Napi::Boolean::New(env, (*pDomain) < (*other->pDomain));
    }

    Napi::TypeError::New(env, "GDomain::operator< : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// bool operator==(const Domain &other) const;
Napi::Value GDomain::operator_eq(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
...
// bool operator!=(const Domain &other) const;
Napi::Value GDomain::operator_nq(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
...
Napi::Value GDomain::NumIntervals(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        return Napi::Number::New(env, pDomain->NumIntervals());
    }

    Napi::TypeError::New(env, "GDomain::NumIntervals : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}

// ClosedInterval operator[](int i) const;
Napi::Value GDomain::operator_brackets(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 1 && info[0].IsNumber())
    {
        ...
        ClosedInterval result = pDomain->operator[](i);
        auto external         = Napi::External<ClosedInterval>::New(env, new ClosedInterval(result));
        return GClosedInterval::constructor.New({external});
    }
...
Napi::Value GDomain::AllValues(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    if (info.Length() == 0)
    {
        Domain result = Domain::AllValues();
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }

    Napi::TypeError::New(env, "GDomain::AllValues : Invalid arguments").ThrowAsJavaScriptException();
    return env.Null();
}
...
Napi::Value GDomain::FromValues(const Napi::CallbackInfo &info)
{
    ...
    if (info.Length() == 1 && info[0].IsArray())
    {
        ...
        for (uint32_t i = 0; i < values.Length(); i++)
        {
            ...
            if (!value.IsNumber())
            {
                Napi::TypeError::New(env, "GDomain::FromValues : Invalid arguments").ThrowAsJavaScriptException();
                return env.Null();
            }
            vec.push_back(value.As<Napi::Number>().Int64Value());
        }
        Domain result = Domain::FromValues(vec);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }
...
Napi::Value GDomain::FromIntervals(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    ...
    {
        Napi::Array intervals = info[0].As<Napi::Array>();
        std::vector<ClosedInterval> vec;
        for (uint32_t i = 0; i < intervals.Length(); i++)
        {
            auto interval = intervals.Get(i);
            if (!interval.IsObject() || !interval.As<Napi::Object>().InstanceOf(GClosedInterval::constructor.Value()))
            {
                Napi::TypeError::New(env, "GDomain::FromIntervals : Invalid arguments").ThrowAsJavaScriptException();
                return env.Null();
            }
            auto closedInterval = GClosedInterval::Unwrap(interval.As<Napi::Object>());
            vec.push_back(*closedInterval->pClosedInterval);
        }
        Domain result = Domain::FromIntervals(vec);
        auto external = Napi::External<Domain>::New(env, new Domain(result));
        return GDomain::constructor.New({external});
    }
    ...
    : Napi::ObjectWrap<GClosedInterval>(info)
{
    ...
    if (info.Length() == 1 && info[0].IsExternal())
    {
        auto external   = info[0].As<Napi::External<ClosedInterval>>();
        pClosedInterval = dynamic_cast<ClosedInterval *>(external.Data());
...
{
    if (pClosedInterval) delete pClosedInterval;
}
